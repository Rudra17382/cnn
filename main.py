...
class dataPreProcess():
    ...
    def __init__(self):
        """
        Intializing thr data
        """

        self.screen = genericViewObjects.newScreen("Tune Data Processing")
        self.objects = genericViewObjects(self.screen)
        
        self.fields = []
        self.fieldNames = ("Samples:", "Time:", "MFCCs:", "FFTs:", "HopLen:", "Segments:")
        self.defaultValues = (22050, 30, 40, 2048, 512, 10)
        self.fieldKeys = ("samples", "time", "mfccs", "ffts", "hopLen", "segments")
    ...
    def show(self):
        ...
        self.objects.button("Confirm Hyper Parameters", self.HyperParameters, (self.localRowCounter, 1))
    ...
    def HyperParameters(self):
        ...
        try:
            ...
            for index in range(len(self.fields)):
                TuningHP[self.fieldKeys[index]] = int(self.fields[index].get())
...
class buildCNN():
    ...
    def addLayer(cls, HyperParameters, layer = None):
        ...
        if layer == "addConvolution":
            ...
            cls.addLabel(cls, f"--New Train Parameters--   {HyperParameters}   --New Train Parameters--", cls.showModelFrame)
...
class addConvolutionalLayer():
    ...
    def __init__(self):
        ...
        self.screen = genericViewObjects.newScreen("Convolution and Pool Hyper Parameters")
        self.objects = genericViewObjects(self.screen)

        self.fields = []
        self.fieldNames = ("No of Filters:", "Kernel:", "Activation:", "Pool Kernel:", "Pool Strides:", "Pool Padding:")
        self.defaultValues = (32, str((3, 3)), "relu", str((3, 3)), str((2, 2)), "same")
        self.fieldKeys = ("filters", "kernel", "activation", "pKernel", "pStride", "pPad")
    ...
    def HyperParameters(self):
        ...
        try:
            convHP = {}
            for index in range(len(self.fields)):
                convHP[self.fieldKeys[index]] = (
                        int(self.fields[index].get()) if index == 0 else 
                        str(self.fields[index].get())
                )
...
class addHiddenLayer():
    ...
    def __init__(self):
        ...
        self.screen = genericViewObjects.newScreen("Hidden Dense Layer Hyper Parameters")
        self.objects = genericViewObjects(self.screen)

        self.fields = []
        self.fieldNames = ("Neurons:", "Activation:", "Dropout:")
        self.defaultValues = (128, "relu", 0.5)
        self.fieldKeys = ("neurons", "activation", "dropout") 
    ...
    def HyperParameters(self):
        ...
        try:
            LayerHP = {}
            for index in range(len(self.fields)):
                LayerHP[self.fieldKeys[index]] = (
                        str(self.fields[index].get()) if index == 1 else 
                        float(self.fields[index].get()) if index == 2 else 
                        int(self.fields[index].get())
                    )
...
class trainParameters():
    ...
    def __init__(self):
        ...
        self.screen = genericViewObjects.newScreen("Training Hyper Parameters")
        self.objects = genericViewObjects(self.screen)

        self.fields = []
        self.fieldNames = ("Test set:", "Validation set:", "Learning rate:", "Batch Size:", "Epochs:", "Verbose:", "Early Stop:")
        self.defaultValues = (0.2, 0.2, 0.001, 32, 15, 1, "True")
        self.fieldKeys = ("test", "validate", "lr", "batch", "epochs", "verbose", "earlyStop")
    ...
    def HyperParameters(self):
        ...
        try:
            ...
            for index in range(len(self.fields) -1):
                    specs[self.fieldKeys[index]] = (
                        float(self.fields[index].get()) if index in(0, 1, 2) 
                        else int(self.fields[index].get()) 
                    )

            specs["earlyStop"] = True if (self.fields[-1].get()) == "True" else False
...
class earlyStopping():
    ...
    def __init__(self):
        ...
        self.screen = genericViewObjects.newScreen("Early Callback Parameters")
        self.objects = genericViewObjects(self.screen)

        genreClassifier.earlyStop = True

        self.fields = []
        self.fieldNames = ("Monitor:", "Min Delta:", "Patience:", "Verbose:", "mode:")
        self.defaultValues = ("val_acc", 0, 2, 1, "auto")
        self.fieldKeys = ("monitor", "delta", "patience", "verbose", "mode")
    ...
    def HyperParameters(self):
        ...
        try:
            ...
            for index in range(len(self.fields)):
                    specs[self.fieldKeys[index]] = (
                        int(self.fields[index].get()) if index in (1, 2, 3)
                        else str(self.fields[index].get())
                    )
...
class createAndTrainModels():
    ...
    def __init__(self, convolutionLayers = None, HiddenLayers = None, model = "custom"):
        ...
        self.objects.label(str(f"The loss of the model is: {genreClassifier.error}"), screen= statsFrame)
